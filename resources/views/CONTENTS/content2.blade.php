...
document.addEventListener('DOMContentLoaded', function() {
    ...
    sendButton.addEventListener('click', function() {
        ...
        .then(data => {
            ...
            // Check if the response has both text and multiple images
            if (data.text && data.img && Array.isArray(data.img)) {
                // If multiple images, display all of them
                const images = data.img.map(imgUrl => `<img src="${imgUrl}" alt="Bot Image" width="200" class="mt-2 rounded-lg">`).join('');
                ...
                botReply = `<div class="text-left my-2"><span class="px-3 py-2 bg-gray-300 rounded-lg">${data.reply}</span></div>`;
            }
